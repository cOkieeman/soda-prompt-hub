...
<style>
  ...
  .comfy-hero-copy::after { content: "RETURN / 05"; position: absolute; right: -8px; bottom: -20px; color: rgba(23,24,21,.06); font: 900 72px/1 monospace; transform: rotate(-5deg); }
  ...
  .comfy-card-image { position: relative; display: block; overflow: hidden; border-bottom: 1px solid var(--line); background: #1e1f1c; }
  .comfy-card-image img { width: 100%; height: 100%; object-fit: contain; }
  ...
  @media (max-width: 1000px) { .comfy-controls, .comfy-grid { grid-template-columns: 1fr 1fr; } .comfy-controls .comfy-panel:last-child { grid-column: 1 / -1; } }
  @media (max-width: 620px) { .comfy-page { width: min(100% - 18px, 1480px); margin-top: 9px; } .comfy-hero, .comfy-controls, .comfy-grid { grid-template-columns: 1fr; } .comfy-hero-copy { padding: 30px 22px; border-right: 0; border-bottom: 1px solid var(--line); } .comfy-flow { padding: 24px 22px; } .comfy-controls .comfy-panel:last-child { grid-column: auto; } .comfy-card { grid-template-rows: 230px auto; } .comfy-actions { grid-template-columns: 1fr; } .comfy-head { display: block; } }
</style>
"""
...
<section class="comfy-page" id="comfyPage" hidden>
  <section class="comfy-hero">
    <div class="comfy-hero-copy"><span class="eyebrow">ComfyUI return archive / local first</span><h1>结果<br>回流</h1><p>把 ComfyUI 出图带回 Mac。这里读取图片里真实存在的 workflow 与生成参数；没有元数据时会明确标出，不会补写或猜测。</p></div>
    <aside class="comfy-flow"><span class="section-label">Four-step review</span><ol><li>导入一张图片，或只读扫描共享结果目录</li><li>核对 seed、模型、LoRA 与 Prompt</li><li>人工选择它属于哪个创作项目</li><li>决定关联、加入候选、记录失败或建立下一版</li></ol></aside>
  </section>
  <section class="comfy-controls">
    <form class="comfy-panel" id="comfyFileForm"><h2>1. 导入图片</h2><label class="comfy-file" for="comfyFile">选择 PNG / JPEG / WebP<input id="comfyFile" type="file" accept="image/png,image/jpeg,image/webp" hidden></label><p id="comfyFileName">尚未选择图片</p><button class="comfy-button signal" type="submit">读取图片元数据</button></form>
    <form class="comfy-panel" id="comfyDirectoryForm"><h2>或：只读扫描目录</h2><label>Mac / SMB 挂载目录<input id="comfyDirectory" placeholder="/Volumes/ComfyUI/output"></label><p>不会移动、改名或覆盖 Windows 输出目录中的文件。</p><button class="comfy-button" type="submit">扫描目录</button></form>
    <div class="comfy-panel"><h2>2. 选择关联项目</h2><label>创作项目<select id="comfyProject"><option value="">请选择项目</option></select></label><p>所有处理动作都需要先选择项目；“记录失败”除外。</p><button class="comfy-button secondary" id="comfyRefresh" type="button">刷新回流库</button></div>
  </section>
  <div class="comfy-status" id="comfyStatus">回流库准备就绪。先导入图片或扫描结果目录。</div>
  <div class="comfy-head"><div><span class="section-label">Local result archive</span><h2>待审核结果</h2></div><label>显示<select id="comfyFilter"><option value="">全部</option><option value="unreviewed">未审核</option><option value="candidate">数据集候选</option><option value="failed_test">失败测试</option><option value="reference">已关联参考</option></select></label></div>
...
(() => {
  ...
  function metadataMarkup(result) {
    ...
    if(!result.metadata_present) return '<div class="comfy-meta-summary comfy-no-meta"><strong>无生成元数据</strong><br>图片可以保留并人工关联，但不会显示伪造参数。</div>';
    ...
    return `<div class="comfy-meta-summary"><strong>${escapeHtml(metadata.checkpoint||'未识别模型')}</strong><br>Seed ${escapeHtml(value(metadata,'seed'))} · Steps ${escapeHtml(value(metadata,'steps'))} · CFG ${escapeHtml(value(metadata,'cfg'))}<br>${escapeHtml(metadata.sampler||'未识别 sampler')} / ${escapeHtml(metadata.scheduler||'未识别 scheduler')}<br>LoRA: ${escapeHtml(loras)}${prompts.length?`<p class="comfy-prompt">${escapeHtml(prompts.join('\n'))}</p>`:''}</div>`;
  }
  function card(result) {
    const associated=(result.associations||[]).map(item=>item.id).join(' · ');
    return `<article class="comfy-card" data-comfy-result="${escapeHtml(result.result_id)}"><a class="comfy-card-image" href="${escapeHtml(result.original_url)}" target="_blank" rel="noopener"><img src="${escapeHtml(result.thumbnail_url)}" alt="${escapeHtml(result.filename)}"><span class="comfy-disposition">${escapeHtml(labels[result.disposition]||result.disposition)}</span></a><div class="comfy-card-body"><div class="comfy-card-title"><strong>${escapeHtml(result.filename)}</strong><span>${result.width}×${result.height}</span></div>${metadataMarkup(result)}${associated?`<p>已关联：${escapeHtml(associated)}</p>`:''}<details class="comfy-details"><summary>查看完整原始 metadata</summary><pre>${escapeHtml(JSON.stringify(result.metadata||{},null,2))}</pre></details><div class="comfy-actions"><button class="comfy-button secondary" data-comfy-action="attach">只关联项目</button><button class="comfy-button signal" data-comfy-action="candidate">加入数据集候选</button><button class="comfy-button secondary" data-comfy-action="failed">记录失败测试</button><button class="comfy-button" data-comfy-action="branch">由此建立下一版</button></div></div></article>`;
  }
  function render() { const filter=$('#comfyFilter').value; const items=state.results.filter(item=>!filter||item.disposition===filter); $('#comfyGrid').innerHTML=items.length?items.map(card).join(''):'<div class="comfy-empty"><div><strong>这里还没有对应的回流图片</strong><p>导入图片后会先进入“未审核”。</p></div></div>'; }
  function status(message) { $('#comfyStatus').textContent=message; }
  async function load() { [state.results,state.projects]=await Promise.all([api('/api/comfy-results?limit=300'),api('/api/creative/projects?limit=100')]); const selected=$('#comfyProject').value; $('#comfyProject').innerHTML='<option value="">请选择项目</option>'+state.projects.map(item=>`<option value="${escapeHtml(item.project_id)}">V${item.lineage?.iteration||1} · ${escapeHtml(item.title)}</option>`).join(''); if(state.projects.some(item=>item.project_id===selected)) $('#comfyProject').value=selected; render(); }
  ...
  async function act(resultId, action) { const projectId=$('#comfyProject').value; if(action!=='failed'&&!projectId) return status('请先在上方选择这张图所属的创作项目。'); if(action==='failed') { await api(`/api/comfy-results/${encodeURIComponent(resultId)}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({disposition:'failed_test'})}); status('已记录为失败测试；图片没有进入任何训练集。'); }
    else if(action==='branch') { const child=await api(`/api/comfy-results/${encodeURIComponent(resultId)}/branch/${encodeURIComponent(projectId)}`,{method:'POST'}); status(`已建立 ${child.title}；来源 workflow 和参数保存在版本链。`); }
    else { const endpoint=action==='candidate'?'candidate':'attach'; await api(`/api/comfy-results/${encodeURIComponent(resultId)}/${endpoint}/${encodeURIComponent(projectId)}`,{method:'POST'}); status(action==='candidate'?'已关联并加入数据集候选；仍需在创作台审核 caption。':'已关联项目；图片尚未进入数据集。'); }
    await load(); }
  ...
  $('#comfyRefresh').addEventListener('click',()=>load().then(()=>status('回流库已刷新。')).catch(error=>status(error.message)));
